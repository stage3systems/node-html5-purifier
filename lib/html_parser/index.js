...
function parsePurified(htmlInput, prefix, postfix, cb) {
  ...
  if (typeof prefix !== 'undefined' || typeof postfix !== 'undefined') {
    async.whilst(function() {
      return (htmlInput.indexOf(prefix) > -1 || htmlInput.indexOf(postfix) > -1);
    }, function(cb) {
      namespacer.stripNamespace(htmlInput, prefix, postfix, function(err, reverted) {
        htmlInput = reverted;
        cb();
      });
    }, function(err) {
      if (err) return cb(err);
      return cb(null, htmlInput);
    });
  } else {
    cb(null, htmlInput);
  }
