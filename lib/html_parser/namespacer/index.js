...
function stripNamespace(htmlInput, prefix, postfix, cb) {
    ...
    startTag: function(tagNameLowerCase, attribs, outputCache) {
      ...
      if (!blacklisted) {
        ...
        for (var i = 0; i < attribs.length; i += 2) {
          ...
          var value = attribs[i + 1];
          // if parsing an "id" that has content
          if ((key === 'id' || key === 'class') && !value) {
            hasClass = false;
          }
          else if (key === 'id' && value.indexOf(prefix) > -1) {
            outputCache.push(' id="' + value.replace(prefix, '') + '"');
            replaced = true;
          }

          // if parsing an "class" that has content
          else if (key === 'class' && (value.indexOf(prefix) > -1 || value.indexOf(postfix) > -1)) {
            hasClass = true;
            replaced = true;
            value = value.replace(postfix, '').trim();
            outputCache.push(' class="');
            var classTokens = value.split(' ');
            var classes = [];
            for (var j = 0; j < classTokens.length; j++) {
  ...
  // if all class values are removed, remove class="" too.
  if (!hasClass) {
    revertedHtml = revertedHtml.replace('class = ""', '');
  }
